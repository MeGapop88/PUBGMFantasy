/**
 * PREDICTIONS PAGE
 * Dual-pane Tactical Prediction interface matching Stitch "Match Prediction (Desktop)"
...
export function renderPredictions(store, router) {
  ...
  function renderContent() {
    predictions = getUserPredictions();
    const currentMatch = matches.find(m => m.id === selectedMatchId) || matches[0];
    const currentPred = predictions[currentMatch.id];
    
    if (!selectedTeamId) {
      selectedTeamId = currentPred?.predictedTeamId || currentMatch.winner?.teamId || currentMatch.teams[0]?.teamId;
    }

    const activeTeam = currentMatch.teams.find(t => String(t.teamId) === String(selectedTeamId)) || currentMatch.teams[0];
    const isPicked = currentPred && String(currentPred.predictedTeamId) === String(activeTeam.teamId);
          ...
          ${matches.map(m => {
            ...
            return `
              <button class="match-select-btn px-4 py-2.5 shrink-0 hud-card text-left transition-all border ${isSelected ? 'border-primary bg-primary/10' : 'border-outline-variant hover:border-outline'}" data-match-id="${m.id}">
                ...
                <div class="font-label text-[10px] text-outline uppercase">WINNER: ${m.winner?.teamName ?? '—'}</div>
              </button>
            `;
          }).join('')}
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div class="lg:col-span-5 hud-card border border-outline-variant bg-[#1A1A1C] relative overflow-hidden flex flex-col">
            ...
            <div class="p-3 flex flex-col gap-2 max-h-[600px] overflow-y-auto" id="team-list">
              ${[...currentMatch.teams].sort((a,b) => a.rank - b.rank).map(team => {
                const isSelected = String(team.teamId) === String(activeTeam.teamId);
                  ...
                  <button class="team-select-btn p-3 border transition-all flex items-center gap-4 text-left relative group ${isSelected ? 'border-primary bg-primary/10' : 'border-outline-variant hover:border-outline bg-[#0E0E0F]'}" data-team-id="${team.teamId}">
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <div class="font-label text-xs text-outline flex items-center gap-3 mt-0.5">
                        <span>FINISH: <strong style="color:${placementColor(team.rank)}">${ordinal(team.rank)}</strong></span>
            ...
            <div class="hud-card border border-outline-variant bg-[#1A1A1C] p-6 md:p-8 relative overflow-hidden">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <div class="hud-card p-3 text-center bg-[#0E0E0F]">
                  ...
                  <div class="font-headline font-bold text-2xl" style="color:${placementColor(activeTeam.rank)}">${ordinal(activeTeam.rank)}</div>
                </div>
                <div class="hud-card p-3 text-center bg-[#0E0E0F]">
                  <div class="font-label text-[10px] font-bold text-outline uppercase tracking-wider mb-1">TOTAL KILLS</div>
                  <div class="font-headline font-bold text-white text-2xl">${activeTeam.totalKills}</div>
                </div>
                <div class="hud-card p-3 text-center bg-[#0E0E0F]">
                  <div class="font-label text-[10px] font-bold text-outline uppercase tracking-wider mb-1">TOTAL DAMAGE</div>
                  <div class="font-headline font-bold text-white text-2xl">${fmt(activeTeam.totalDamage)}</div>
                </div>
                <div class="hud-card p-3 text-center bg-[#0E0E0F]">
                  <div class="font-label text-[10px] font-bold text-outline uppercase tracking-wider mb-1">PAYOUT VALUE</div>
                  <div class="font-headline font-bold text-primary text-2xl">${PLACEMENT_PTS[activeTeam.rank] ?? 0} PTS</div>
                </div>
              </div>
              ...
              <button id="confirm-pick-btn" class="btn-primary w-full py-4 font-headline font-bold text-lg uppercase tracking-widest flex items-center justify-center gap-2">
                ...
                ${isPicked ? `PICK CONFIRMED FOR ${activeTeam.teamName} (${currentPred.points} PTS)` : `LOCK IN PREDICTION FOR ${activeTeam.teamName}`}
              </button>
            </div>
    ...
    document.getElementById('confirm-pick-btn').onclick = () => {
      try {
        submitPrediction(currentMatch.id, activeTeam.teamId, activeTeam.teamName);
